...
class App extends Component {
  ...
  deleteSong = id => {
    ...
    const filterPlaylist = playlist.filter(eachSong => eachSong.id !== id)
    this.setState({playlist: filterPlaylist})
  }

  render() {
    const {searchInput, playlist} = this.state
    const songFound = playlist.filter(eachSong =>
      ...
      <div className="main-container">
        ...
        <div className="playlist-container">
          ...
          {songFound.length === 0 ? (
            ...
            <ul className="initialTracksList-container">
              {songFound.map(eachTrack => (
                <SongsPlaylist
                  key={eachTrack.id}
                  eachTrackDetails={eachTrack}
